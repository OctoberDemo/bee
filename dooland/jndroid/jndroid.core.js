...
Math.calc_I = function(values) {
    ...
    for (var i = 0; i < values.length; i++) {
        ...
        if (v > 0) {
            allZero = false;
            ent += v * (Math.log(values[i]) / Math.log(2));
        }
    }
    if (allZero) {
        return 0;
    }
    var result = 1 + ent / (Math.log(values.length) / Math.log(2));
    result = Math.max(0, result);
    result = Math.numberFixed(result, 3);
    return result;
};
...
function View() {
	...
	*/
    ...
    this.onBeforeDraw = function(canvas) {

    };

    this.onAfterDraw = function(canvas) {
    };
	...
	*/
    this.setWillNotDraw = function(willnotdraw) {
        ...
        if (willNotDraw === false) {
            ...
            HTMLCanvas.style.borderRadius = this.div.style.borderRadius;
