...
function MIconButton() {
    ...
    this.onLayout = function() {
        img.style.left = ((this.getMW() - imgW) / 2) + "px";
        img.style.top = ((this.getMH() - imgH) / 2) + "px";
    }
}
...
function addWave(obj) {
    obj.downX = 0;
    obj.downY = 0;
    obj.bg = new WaveDrawable();
    obj.bg.setCallback(obj);

    obj.setWillNotDraw(false);

    obj.setDimBg = function(dim) {
        this.bg.setDimBg(dim);
    };

    obj.setWaveColor = function(c) {
        this.bg.setWaveColor(c);
    };

    obj.onTouchEvent = function(e) {
        switch (e.getAction()) {
            case ME.ACTION_DOWN:
                this.downX = e.getX();
                this.downY = e.getY();
    ...
    obj.onAfterDraw = function(canvas) {
        var bg = this.bg;
        var density = DisplayMetrics.density;
        if (this.isPressed()) {
            bg.setState(View.VIEW_STATE_PRESSED);
            bg.setX(this.downX * density);
            bg.setY(this.downY * density);
        } else {
...
function _MDialog() {
    ...
    this.setContent = function(cnt) {
        var index = this.indexOfChild(msg);
        this.removeView(msg);
        this.addView(cnt, index, mMsgLp);
        return this;
    };
...
function MCheckBox() {
    ...
    var isChecked = false;
    var checkListener = null;

    var unCheckNode = document.createElement("div");
    unCheckNode.style.width = "18px";
    unCheckNode.style.height = "18px";
    unCheckNode.style.borderRadius = "2px";
    unCheckNode.style.borderWidth = "2px";
    unCheckNode.style.borderStyle = "solid";
    unCheckNode.style.borderColor = Utils.toCssColor(0x80000000);

    var pressNode = document.createElement("div");
    pressNode.style.width = "18px";
    pressNode.style.height = "18px";
    pressNode.style.borderRadius = "2px";
    pressNode.style.background = Utils.toCssColor(color);

    var checkNode = document.createElement("div");
    checkNode.style.width = "18px";
    checkNode.style.height = "18px";
    checkNode.style.borderRadius = "2px";
    checkNode.style.background = Utils.toCssColor(color);
    var check = document.createElement("div");
    check.style.width = "6px";
    check.style.height = "12px";
    check.style.borderWidth = "2px";
    check.style.borderStyle = "solid";
    check.style.borderTop = 0;
    check.style.borderLeft = 0;
    check.style.position = "absolute";
    check.style.left = "6px";
    check.style.top = "2px";
    check.style.borderColor = Utils.toCssColor(0xfff0f0f0);
    check.style.transform = "rotate(45deg)";
    checkNode.appendChild(check);

    addWave(this);
    this.setCornerSize(100);

    this.setColor = function(c) {
        pressNode.style.background = Utils.toCssColor(c);
        checkNode.style.background = Utils.toCssColor(c);
    };
    ...
    this.setChecked = function(c) {
        isChecked = c;
        if (checkListener) {
            checkListener.call(this, c);
        }
        this.postInvalidate();
    };

    this.onDraw = function() {
        removeNode(this.div, pressNode);
        removeNode(this.div, checkNode);
        removeNode(this.div, unCheckNode);

        if (this.isPressed()) {
            this.div.appendChild(pressNode);
        } else {
            if (isChecked) {
                this.div.appendChild(checkNode);
            } else {
                this.div.appendChild(unCheckNode);
            }
        }

        function removeNode(div, node) {
            if (node.parentNode) {
    ...
    this.onLayout = function() {
        var x = (this.getMW() - 18) / 2;
        var y = (this.getMH() - 18) / 2;
        unCheckNode.style.left = x + "px";
        unCheckNode.style.top = y + "px";

        pressNode.style.left = x + "px";
        pressNode.style.top = y + "px";

        checkNode.style.left = x + "px";
        checkNode.style.top = y + "px";
...
function WaveDrawable() {
    ...
    this.drawPress = function(c) {
        ...
        var a = Math.max(temp * alpha, 0);
        var color = Color.argb(a, Color.red(waveColor), Color.green(waveColor), Color.blue(waveColor));
