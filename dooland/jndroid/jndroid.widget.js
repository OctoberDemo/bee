...
function ScrollView() {
    ...
    jndroid.style.fontFamily = "微软雅黑,Roboto,sans-serif";
    ...
    jndroid.style.display = "none";
    this.div.appendChild(jndroid);

    this.showJndroidLogo = function() {
        jndroid.style.display = "block";
    };

    this.getYProcessor = function() {
        return yProcessor;
    };
    ...
    this.flingTo = function(y) {
        var scrollY = this.getScrollY();
        if (y != scrollY) {
            var maxScroll = this.getChildAt(0).getMH() - this.getMH();
            var yProcessor = this.getYProcessor();
            yProcessor.setAccelerator(0.05);
            yProcessor.flingDistance(scrollY, scrollY - y, 0, maxScroll);
            yProcessor.setEndListener(function() {
                yProcessor.setAccelerator(0.0015);
            });
            this.postInvalidate();
        }
    };
    ...
    function FlingProcessor() {
        ...
        this.setAccelerator = function(_a) {
            a = _a;
        };
...
function ScrollBar(scrollView) {
    ...
    scrollView.addScrollChangedListener(function(x, oldx, y, oldy) {
        ...
        var process = y / (scrollChild.getMH() - h);
        var offset = process * (h - thumbH);
        thumb.layoutByTransform(0, offset);
    });
    ...
    this.onTouchEvent = function(e) {
        ...
        thumb.layoutByTransform(0, y);

        var processor = scrollView.getYProcessor();
...
function WebView() {
    ...
    this.setOnDomLoadedListener = function(l) {
        var id = setInterval(function() {
            if (frame.contentWindow != null && frame.contentWindow.document != undefined) {
                clearInterval(id);
                setTimeout(function() {
                    l.call(this);
                }, 1000);
